...
const BlogTitles = () => {
  ...
  const [selectedCategory, setSelectedCatgeory] = useState('General');
 ...
 const onSubmitHandler = async (e) => {
  ...
  try {
    ...
    if (data.success) {
      setContent(data.content); // use 'content' as returned by your backend
    } else {
      ...
      >
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {blogCategories.map((item) => (
            <span
              onClick={() => setSelectedCatgeory(item)}
